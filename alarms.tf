...
resource "aws_cloudwatch_metric_alarm" "burst_balance_too_low" {
  ...
  alarm_actions             = concat([aws_sns_topic.default.arn], var.additinal_alert_actions)
  ok_actions                = concat([aws_sns_topic.default.arn], var.additinal_ok_actions)
  insufficient_data_actions = var.additional_insufficient_data_actions
